...
import { CadastrarAutomovelModel, CadastrarAutomovelResponseModel, CombustivelEnum } from '../automovel.models';
...
@Component({
  ...
  templateUrl: './cadastrar-automovel.html',
})
export class CadastrarAutomovel {
  ...
  protected automovelForm: FormGroup = this.formBuilder.group({
    //placa: ['', [Validators.required,Validators.pattern(/^[A-Z]{3}[0-9][A-Z][0-9]{2}$/i)]],
  ...
  public cadastrar() {
    ...
    const automovelModel: CadastrarAutomovelModel = this.automovelForm.value;

    const cadastroObserver: Observer<CadastrarAutomovelResponseModel> = {
      next: () =>
        this.notificacaoService.sucesso(
          `O registro "${automovelModel.modelo}" foi cadastrado com sucesso!`
        ),
      error: (err) => this.notificacaoService.erro(err),
      complete: () => this.router.navigate(['/automoveis']),
    };

    this.automovelService.cadastrar(automovelModel).subscribe(cadastroObserver);
  }
}
